...
async def live(equipment_id: str, _: dict = Depends(authorize(Permission.GENERAL_READ))):
    return generate_live_reading(_require_equipment(equipment_id))


@router.get("/trend/{equipment_id}")
async def trend(equipment_id: str, hours: int = Query(24, ge=1, le=168), _: dict = Depends(authorize(Permission.GENERAL_READ))):
...
async def fleet_status(_: dict = Depends(authorize(Permission.GENERAL_READ))):
...
async def active_alarms(_: dict = Depends(authorize(Permission.GENERAL_READ))):
    alarms = []
