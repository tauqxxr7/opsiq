...
export default function AnalyticsPage() {
  ...
  if (error) return <div className="space-y-6"><PageHeader eyebrow="Reliability engineering" title="Reliability analytics" description="Derived metrics from synthetic work-order evidence." /><ErrorState message={error} /></div>;
  if (!data) return <div className="space-y-6"><PageHeader eyebrow="Reliability engineering" title="Reliability analytics" description="Derived metrics from synthetic work-order evidence." /><Panel><LoadingState message="Computing reliability metrics..." /></Panel></div>;
  ...
  return <div className="page-enter page-enter-active space-y-6">
    <PageHeader eyebrow="Reliability engineering" title="Reliability analytics" description="Derived from synthetic work-order dates and downtime; availability is an illustrative estimate." />
    ...
    <div className="grid gap-6 xl:grid-cols-2"><Panel title="Monthly downtime" description="Last 12 recorded months"><div className="h-72"><ResponsiveContainer><BarChart data={trends}><CartesianGrid stroke={tokens.color.border} vertical={false} /><XAxis dataKey="month" tick={{ fill: tokens.color.muted, fontSize: 11 }} /><YAxis tick={{ fill: tokens.color.muted, fontSize: 11 }} /><Tooltip /><Bar dataKey="downtime_hours" fill={tokens.color.accent} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div></Panel><Panel title="Failure modes" description="Failure records grouped by dominant equipment mode"><div className="h-72"><ResponsiveContainer><PieChart><Pie data={failures} dataKey="value" nameKey="name" innerRadius={55} outerRadius={90} fill={tokens.color.success} label /><Tooltip /></PieChart></ResponsiveContainer></div></Panel></div>
