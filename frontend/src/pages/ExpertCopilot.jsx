import { FileText, Globe2, Send } from "lucide-react";
...
const languages = [
  ["en", "English", ""],
  ["hi", "\uD83C\uDDEE\uD83C\uDDF3 \u0939\u093f\u0928\u094d\u0926\u0940", "[target_language=hi] "],
  ["mr", "\u092e\u0930\u093e\u0920\u0940", "[target_language=mr] "],
  ["ta", "\u0ba4\u0bae\u0bbf\u0bb4\u0bcd", "[target_language=ta] "],
  ["te", "\u0c24\u0c46\u0c32\u0c41\u0c17\u0c41", "[target_language=te] "],
];
...
export default function ExpertCopilot() {
  ...
  const send = async (retryQuery = "", appendQuestion = true) => {
    ...
    const languagePrefix = languages.find(([code]) => code === language)?.[2] ?? "";
    const query = retryQuery || (languagePrefix + displayQuery).trim();
    if (!query || loading) return;
    clearTimeout(retryTimer.current);
    setWarmup(null);
    if (!retryQuery) setInput("");
    setLoading(true);
    if (appendQuestion) setMessages((current) => [...current, { role: "user", content: displayQuery }]);
    try {
      const data = await ask({ query });
      setMessages((current) => [...current, { role: "assistant", content: data.answer, citations: data.citations || [], confidence: data.confidence, detected_language: data.detected_language, query_translated: data.query_translated }]);
    } catch (error) {
    ...
    <div className="grid min-h-[650px] gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
      <section className="card-hover flex min-h-[650px] flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-panel">
        ...
        <div className="scrollbar-thin flex-1 space-y-6 overflow-y-auto p-5 sm:p-6">
          ...
          {messages.map((message, index) => {
            ...
            return <article key={`${message.role}-${index}`} className={`flex items-start gap-3 ${isUser ? "ml-auto max-w-[80%] flex-row-reverse" : "max-w-[90%]"}`}>
              ...
              <div className="min-w-0 flex-1">
                ...
                {message.query_translated && <div className="mb-2 flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 rounded-full border border-primary/30 bg-primary/15 px-2 py-0.5 font-mono text-xs text-primary"><Globe2 size={12} aria-hidden="true" />{message.detected_language} &rarr; EN &rarr; {message.detected_language}</span><span className="text-xs text-muted">Query translated for retrieval</span></div>}<div className={`px-4 py-3 text-sm leading-relaxed text-text-primary ${isUser ? "rounded-xl rounded-tr-sm border border-primary/25 bg-primary/15" : "rounded-xl rounded-tl-sm border border-border bg-card"}`}>{message.content}</div>
          ...
          {warmup && !loading && <div className="flex items-center gap-3 rounded-xl border border-warning/30 bg-warning/10 px-4 py-3" role="status" aria-live="polite"><div className="h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-warning border-t-transparent" /><div className="min-w-0 flex-1"><p className="text-sm font-medium text-warning">Models warming up</p><p className="mt-0.5 text-xs text-text-secondary">First request takes 60ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“90 seconds. Retrying automatically in 20 seconds...</p></div><button type="button" onClick={() => send(warmup.query, false)} className="shrink-0 rounded-lg border border-warning/30 px-3 py-1.5 text-xs font-semibold text-warning transition-colors hover:bg-warning/10">Retry now</button></div>}
        </div>
        <div className="border-t border-border/70 bg-card/50 p-4">
          <div className="flex flex-col gap-2 sm:flex-row">
            <label className="sr-only" htmlFor="copilot-language">Response language</label>
            <select id="copilot-language" value={language} onChange={(event) => setLanguage(event.target.value)} className="rounded-lg border border-border bg-surface px-3 py-3 text-xs text-text-primary outline-none transition-colors focus:border-primary focus:ring-1 focus:ring-primary">{languages.map(([code, label]) => <option key={code} value={code}>{label}</option>)}</select>
          ...
          <p className="mt-2 font-mono text-xs text-muted">Retrieves from indexed documents only. All answers include source citations.{noDocuments && <span className="ml-2 text-warning">No documents indexed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â upload files in Document Library</span>}</p>
        </div>
      </section>
