...
const languages = [
  ["en", "English", ""],
  ["hi", "India - Hindi", "[target_language=hi] "],
  ["mr", "Marathi", "[target_language=mr] "],
  ["ta", "Tamil", "[target_language=ta] "],
  ["te", "Telugu", "[target_language=te] "],
];
    ...
    <div className="grid min-h-[650px] gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
      <section className="card-hover flex min-h-[650px] flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-panel">
        ...
        <div className="scrollbar-thin flex-1 space-y-6 overflow-y-auto p-5 sm:p-6">
          ...
          {warmup && !loading && <div className="flex items-center gap-3 rounded-xl border border-warning/30 bg-warning/10 px-4 py-3" role="status" aria-live="polite"><div className="h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-warning border-t-transparent" /><div className="min-w-0 flex-1"><p className="text-sm font-medium text-warning">Models warming up</p><p className="mt-0.5 text-xs text-text-secondary">First request takes 60 to 90 seconds. Retrying automatically in 20 seconds...</p></div><button type="button" onClick={() => send(warmup.query, false)} className="shrink-0 rounded-lg border border-warning/30 px-3 py-1.5 text-xs font-semibold text-warning transition-colors hover:bg-warning/10">Retry now</button></div>}
        </div>
