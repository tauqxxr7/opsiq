...
export default function ExpertCopilot() {
  ...
  return <div className="page-enter page-enter-active space-y-6">
    ...
    <div className="grid min-h-[650px] gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
      <section className="card-hover flex min-h-[650px] flex-col overflow-hidden rounded-lg border border-border bg-surface shadow-panel">
        ...
        <div className="border-t border-border bg-card p-4">
          <div className="flex gap-2">
            ...
            <input id="copilot-input" className="min-w-0 flex-1 rounded-md border border-border bg-surface px-4 py-3 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === "Enter" && send()} placeholder="Ask an evidence-grounded operational question..." />
            <button onClick={send} disabled={loading || !input.trim()} className="flex min-w-12 items-center justify-center rounded-md bg-primary px-4 text-white hover:bg-primary/90 disabled:opacity-40" aria-label="Send question">
              {loading ? <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" /> : <Send size={18} />}
            </button>
          </div>
          <p className="mt-2 font-mono text-xs text-muted">
            Retrieves from indexed documents only. All answers include source citations.
            {noDocuments && <span className="ml-2 text-warning">⚠ No documents indexed — upload files in Document Library</span>}
          </p>
        </div>
      </section>
      <aside className="card-hover rounded-lg border border-border bg-surface p-5 shadow-panel">
