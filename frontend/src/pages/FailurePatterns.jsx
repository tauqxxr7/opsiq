...
export default function FailurePatterns() {
  ...
  return <div className="page-enter page-enter-active space-y-6">
    ...
    {!loading && data?.status === "ok" && <>
      ...
      <Panel title="Ranked recurring patterns" description={Object.entries(data.source_counts).map(([name, count]) => `${name}: ${count}`).join(" Â· ")} flush>
        <div className="overflow-x-auto"><table className="data-table w-full min-w-[900px]"><thead className="bg-card"><tr><th>Equipment</th><th>Failure mode</th><th>Risk</th><th>Observed range</th><th>Occurrences</th><th>Downtime</th><th>Evidence</th></tr></thead><tbody>{data.patterns.map((pattern) => <tr className="evidence-row" key={`${pattern.equipment_id}-${pattern.failure_type}`}><td className="font-mono font-medium text-primary">{pattern.equipment_id}</td><td className="font-medium">{pattern.failure_type}</td><td><StatusBadge tone={pattern.risk === "CRITICAL" ? "red" : "amber"}>{pattern.risk}</StatusBadge></td><td className="whitespace-nowrap text-text-secondary">{pattern.first_seen} â†’ {pattern.last_seen}</td><td className="font-mono">{pattern.occurrences}</td><td className="font-mono">{pattern.cumulative_downtime_hours} h</td><td className="max-w-sm break-words font-mono text-xs text-muted">{pattern.supporting_evidence_ids.join(", ")}</td></tr>)}</tbody></table></div>
      </Panel>
      <Panel title="Recurring cross-asset root causes" description="Shared labels observed across multiple evidence records.">
        <div className="divide-y divide-border">{data.recurring_root_causes.map((item) => <div className="py-3 first:pt-0 last:pb-0" key={item.value}><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-medium">{item.value}</p><span className="text-xs text-muted">{item.occurrences} records Â· {item.equipment_ids.join(", ")}</span></div><p className="mt-1 break-words font-mono text-xs text-text-secondary">Evidence: {item.supporting_evidence_ids.join(", ")}</p></div>)}</div>
      </Panel>
    </>}
  </div>;
}
